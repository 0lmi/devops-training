...
du -h your_file.txt
```
Som when you observe a text file with 5000 Russian symbols, it's size on the disk is 12KB which is 12 228 bytes. Where 9985 bytes are actual size and 2243 bytes are Metadata[^1] and Overhead[^2].
...
8KB is 8192 bytes, where 5000 bytes are the actual file content, and 3192 bytes are taken for Metadata and Overhead.
...
---
[^1]: Information about the file, such as inode data, permissions, timestamps and other assosiated data with the file
[^2]: The ext4 filesystem allocate space in blocks. Even if your file is smaller than the block size, it will occupy a full block. The ramaining space in that block that is not used by the actual file content contributes to the overhead.
